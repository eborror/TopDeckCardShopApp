...
def remove_manager(manager_id: int = Form(...)):
    ...
    cursor = conn.cursor()
    try:
        # Deleting from MANAGER only removes their staff role
        # It does not delete the Customer or their purchase history
        cursor.execute("DELETE FROM MANAGER WHERE MANAGER_ID = %s", (manager_id,))
        conn.commit()
    except Exception as e:
        conn.rollback()
